...
/*exports.facebook = asyncHandler(async(req, res, next) => {
	...
	const sql = `SELECT * FROM "FbUser" WHERE "id" = ?`;
	console.log(sql);

	const statement = await db.preparePromisified(sql);

	const results = await db.statementExecPromisified(statement, [req.user.id]);
...
exports.register = asyncHandler(async(req, res, next) => {
	...
	let sql =
		`INSERT INTO "User" ("firstName", "lastName", "email", "password", "formattedAddress", "isochrone5mCar", "coordinates") VALUES(?, ?, ?, ?, ?, ST_GEOMFROMGEOJSON('${polygons}', 4326), new ST_POINT(${location.coordinates[0]}, ${location.coordinates[1]}).ST_SRID(4326).ST_TRANSFORM( 4326))`;
	console.log(sql);
